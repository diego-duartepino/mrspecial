...
# Load the two files
muestra_path = "/Users/dlaboy/Desktop/duartepino/IRI data/Junio/Categories.csv"
    ...
    # Identify a brand row
        ...
        try:
            output.append({
                ...
                "Market Share (Dollars)": float(row["col_6"]) if pd.notna(row["col_6"]) else 0.0,
                "Market Volume (Selling Units)": float(row["col_9"].replace(',', '')) if pd.notna(row["col_9"]) else 0.0,
                "Market Share (Sell Units)": float(row["col_12"].replace('%', '')) / 100 if pd.notna(row["col_12"]) else 0.0,
                "Retail Price": float(row["col_15"].replace('$', '')) if pd.notna(row["col_15"]) else 0.0,
                "Month": "June",
            })
...
df_sorted_by_category.to_csv("Category-Brand-TM-June2024.csv", index=False)
