...
export default function IriItemsUpload() {
  ...
  // --- backend status (same pill as other pages) ---
  const [apiOk, setApiOk] = useState(null);   // null = unknown
  ...
  const onUpload = async () => {
    ...
    try {
      setBusy(true);
      const body = { kind: "iri_items", source, year, month: normalizeMonth(month) };
      const json = await safeFetchJSON("/api/upload/iri/items", {
        method: "POST",
  ...
  return (
    <Page
      title="IRI · Items"
      subtitle="Upload your monthly Items file and/or trigger processing"
      maxWidth={980}
          ...
          <div className="row">
            ...
            <div style={{ width: "100%" }}>
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                <FileUpload
                  label="Upload IRI Items (.xlsx)"
                  accept=".xlsx"
                  multiple={false}
                  uploadUrl="/api/upload/iri/items"
                  fieldName="file"
